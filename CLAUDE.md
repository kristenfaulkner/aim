...
9. **Adaptive dashboard intelligence** — 3-mode AI (POST_RIDE/MORNING_WITH_PLAN/MORNING_NO_PLAN) via `/api/dashboard/intelligence`, Claude Opus 4.6, server-side cache (`intelligence_cache` table) + frontend SWR cache (sessionStorage), weather+models parallelized, PrepRec-based output format (prepRecs/recoveryRecs with 3-level disclosure)
...
**Today Page (AI-first rebuild, PrepRec architecture)**: Replaces Dashboard V2. Single-column centered layout (680px max-width), PrepRec-based intelligence delivery (3-level progressive disclosure: title→action→evidence with data pills). Two automatic modes: Morning (ReadinessHero→AIBriefing→TodayCard→VitalsStrip→ThisWeek→AskClaude) and Post-Ride (CollapsedMorning→RideSummary→AIBriefing→TodayCard[recovery]→ThisWeek→AskClaude). Three AI modes: MORNING_WITH_PLAN, MORNING_NO_PLAN, POST_RIDE. Components: PrepRec, ReadinessHero (SVG ring + context pills), VitalsStrip (client-computed trends), ThisWeek (7-day TSS bars), RideSummary (metric pills), TodayCard (mode-aware hub with lazy Get Workout), AIBriefing, CollapsedMorning (mini ring), AskClaude, DataGaps. Hooks: useTodayIntelligence, usePrescription (lazy). Legacy Dashboard at /dashboard-legacy.
