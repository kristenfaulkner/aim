...
// DEMO 6B: LIVING NEURAL INTELLIGENCE — CSS/DOM VERSION
// No Canvas — pure React + CSS animations
// Particles, synapse shooting stars, data labels, heartbeat
...
export default function LivingNeuralCSS() {
  const [, forceRender] = useState(0);

  // Generate stable particles once — avoid center zone for text readability
  ...
  // Ambient dots — also avoid center
  ...
  // Shooting star paths (synapse pulses)
  ...
  return (
    <div style={{ position: "relative", width: "100vw", height: "100vh", background: BG, overflow: "hidden", fontFamily: "system-ui, -apple-system, sans-serif" }}>
          ...
          return (
            <line
              ...
              style={{
                ...
                opacity: opacity,
                animation: `heartbeat ${3 + (i % 3)}s ease-in-out infinite`,
      ...
      {particles.map((p) => (
        ...
        >
          ...
          <div style={{
            ...
            animation: `breathe ${p.breatheDuration}s ease-in-out infinite`,
          ...
          <div style={{
            ...
            animation: `pulseRing ${4 + p.id % 5}s ease-out infinite`,
      ...
      {/* ── SHOOTING STARS (SYNAPSE PULSES) ── */}
      {shootingStars.map((s) => {
        // Create SVG path for the shooting star to follow
        const pathD = `M ${s.startX} ${s.startY} L ${s.endX} ${s.endY}`;
        return (
          <div key={`star-${s.id}`} style={{ position: "absolute", inset: 0, zIndex: 4 }}>
            ...
            <div style={{
              ...
              left: `${s.startX}%`,
              top: `${s.startY}%`,
              animation: `shootingStar ${s.duration}s ease-in-out infinite`,
              animationDelay: `${s.delay}s`,
              offsetPath: `path("M ${s.startX * window.innerWidth / 100} ${s.startY * window.innerHeight / 100} L ${s.endX * window.innerWidth / 100} ${s.endY * window.innerHeight / 100}")`,
              offsetRotate: "0deg",
            }} />
            {/* Trail */}
            <div style={{
              ...
              animation: `shootingStarTrail ${s.duration}s ease-in-out infinite`,
              animationDelay: `${s.delay}s`,
              offsetPath: `path("M ${s.startX * window.innerWidth / 100} ${s.startY * window.innerHeight / 100} L ${s.endX * window.innerWidth / 100} ${s.endY * window.innerHeight / 100}")`,
              offsetRotate: "auto",
      ...
      <div style={{ position: "absolute", bottom: 32, left: 32, zIndex: 10 }}>
        <div style={{ fontSize: 11, color: `${ACCENT}80`, fontFamily: "monospace", letterSpacing: "0.1em", marginBottom: 4 }}>DEMO 6B</div>
        <div style={{ fontSize: 20, color: "rgba(255,255,255,0.9)", fontWeight: 700 }}>Living Neural Intelligence</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>CSS-powered — synapses, heartbeat, data mesh</div>
      </div>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 6, pointerEvents: "none" }}>
        <div style={{ textAlign: "center" }}>
          <h1 style={{ fontSize: 64, fontWeight: 800, color: "white", letterSpacing: "-0.04em", lineHeight: 1.05, margin: 0 }}>
            Your AI<br />
            <span style={{ background: `linear-gradient(135deg, ${ACCENT}, ${BLUE})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Performance Coach</span>
          </h1>
          <p style={{ fontSize: 18, color: "rgba(255,255,255,0.45)", marginTop: 24 }}>Your data, alive and thinking</p>
        </div>
      </div>
    </div>
  );
