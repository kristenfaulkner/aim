          ...
          {!isMobile && (
            <div style={{ display: "flex", gap: 3 }}>
              {["Dashboard", "Sleep", "Health Lab", "Connect", "Settings"].map(item => (
                ...
                }} style={{ background: item === "Sleep" ? T.accentDim : "none", border: "none", padding: "5px 12px", borderRadius: 7, fontSize: 11, fontWeight: 600, color: item === "Sleep" ? T.accent : T.textSoft, cursor: "pointer", fontFamily: font, display: "flex", alignItems: "center", gap: 4 }}>{item === "Settings" ? <><Settings size={12} /> {item}</> : item}</button>
              ))}
            </div>
          )}
        </div>
